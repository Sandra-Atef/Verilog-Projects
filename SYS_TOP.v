...
RST_SYNC # (.NUM_OF_STAGES(2)) U0_RST_SYNC (
.RST(RST_N),
.CLK(UART_CLK),
.SYNC_RST(SYNC_UART_RST)
);

RST_SYNC # (.NUM_OF_STAGES(2)) U1_RST_SYNC (
.RST(RST_N),
.CLK(REF_CLK),
...
DATA_SYNC # (.BUS_WIDTH(8)) U0_ref_sync (
.CLK(REF_CLK),
.RST(SYNC_REF_RST),
...
.CLK(REF_CLK),
.RST(SYNC_REF_RST),
.REGFILE_RdData(RF_RdData),
.REGFILE_RdData_Valid(RF_RdData_VLD),
.REGFILE_WrEN(RF_WrEn),
.REGFILE_RdEN(RF_RdEn),
.REGFILE_Address(RF_Address),
.REGFILE_WrData(RF_WrData),
.ALU_EN(ALU_EN),
.ALU_FUN(ALU_FUN), 
.ALU_OUT(ALU_OUT),
.ALU_OUT_VALID(ALU_OUT_VLD),  
.ALU_CLK_GATE_EN(CLKG_EN), 
.clk_div_en(CLKDIV_EN),   
.FIFO_FULL(FIFO_FULL),
.RX_P_DATA(UART_RX_SYNC), 
.RX_D_VLD(UART_RX_V_SYNC),
.FIFO_WR_DATA(UART_TX_IN), 
.FIFO_WR_INC(UART_TX_VLD)
);
